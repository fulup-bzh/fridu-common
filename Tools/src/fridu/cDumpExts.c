/* 
 * winDumpExts.c --
 * Author:   Gordon Chaffee, Scott Stanton, [Hacked:Fulup Ar Foll]
 *
 ...
 *
 * 26/05/98  Modified by Fulup Ar Foll inorder to be compliant with
 *           fridu Unix/NT rules. Add directory prefix and change @ facility
 *           new syntaxe is:
 *            winDumpExt DestinationFile libName ObjSourceDir @fileWithObjName 
 *
 ...
 */
// Force Windows to include 2000lines of specifics [hug!!!]
#undef _WINDOWS_
#include <windows.h>
#include <stdio.h>
#include <string.h>
 ...
 *----------------------------------------------------------------------
 */
LPCTSTR GetSZStorageClass (BYTE storageClass)
{
 ...
 */
void
DumpFile(LPCTSTR filename, FILE *fout, int full)
{
    HANDLE hFile;
    HANDLE hFileMapping;
    LPVOID lpFileBase;
...
main(int argc, char **argv)
{
    ...
    if (argc < 3) {
      Usage:
	fprintf(stderr, "Usage: %s ?-verbose? ?-oOutfile? ?-f(ull)? -dDllname @objFileList ..\n", argv[0]);
	exit(1);
    }
	...
	if (!strcmp(argv[arg], "--")) {
	    ...
	    break;
            ...
            if (verbose) fprintf (stderr,"verbose mode on\n");
	    verbose= 1;
	} else if (!strncmp(argv[arg],"-f",2)) {
            if (verbose) fprintf (stderr,"full mode on\n");
	    full = 1;
	} else if (!strncmp(argv[arg],"-d",2)) {
            dllname = &argv[arg][2];
            if (verbose) fprintf (stderr,"dll name=%s\n",dllname);
	} else if (!strncmp(argv[arg],"-o",2)) {
	    outfile = &argv[arg][2];
            if (verbose) fprintf (stderr,"out file=%s\n",outfile);
        } else if (!strncmp(argv[arg],"-P",2)) {
 	  prefixDir = &argv[arg][2];
          if (verbose) fprintf (stderr,"object prefix dir=%s\n", prefixDir);
	} else {
    ...
    for (; arg < argc; arg++) {
        // search object file path in @file 
        if (argv[arg][0] == '@') {
		...
		exit(1);
            ...
            if (verbose) fprintf (stderr,"object file input list=%s\n", cmdLine);
            
            // if prefix exist we prepare a file with full path to object file for DLL linker
	    if (prefixDir != NULL) {
              sprintf (cmdLine,"%s/%s.lst",prefixDir,dllname);
	      fdll = fopen(cmdLine, "w");
	      if (fdll == NULL) {
		fprintf(stderr, "Unable to open \'%s\' for writing\n",cmdLine);
		perror("");
		exit(1);
            ...
            if (verbose) fprintf (stderr,"dll object dump fullpath=%s\n", cmdLine);

	    // process each file line
            while (fgets (line, sizeof (line), fargs) != NULL) {
              if (prefixDir != NULL) {
                sprintf (cmdLine,"%s/%s", prefixDir, line);
                fputs (cmdLine,fdll);
              } else {
                sprintf (cmdLine,"%s", prefixDir, line);
              }
	      for (ind=0; cmdLine [ind] != '\0'; ind++) {
		if (cmdLine [ind] == '/') cmdLine [ind] = '\\';
              }
	      // remove last cr from input line
	      if (cmdLine [ind-1] == '\n') cmdLine [ind-1] = '\0';
              if (verbose) { 
                    fprintf (stderr,"getting entry points from [%s]\n", cmdLine);
              }
              DumpFile(cmdLine, fout, full);
            }
	} else {
